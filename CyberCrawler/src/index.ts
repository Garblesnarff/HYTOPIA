...
// Combined imports
import { startServer, Entity, EntityEvent, PlayerEntity, PlayerEvent, World, Player, BlockTypeRegistry, Vector3 } from 'hytopia';
...
startServer((world: World) => { // Add type annotation for world
  ...
  world.on(PlayerEvent.JOINED_WORLD, ({ player }: { player: Player }) => { // Add type annotation for player
    ...
    const playerEntity = setupPlayer(world, player);
    
    // Add debug help commands
    ...
    world.chatManager.registerCommand('/showblocks', (cmdPlayer) => {
      if (cmdPlayer.id === player.id) {
        // Place a test block at player's feet
        const entity = world.entityManager.getPlayerEntitiesByPlayer(player)[0];
        if (entity && entity.position) {
          const pos = entity.position;
